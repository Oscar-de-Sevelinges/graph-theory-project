...
// menu graphqiue pour les préférences choisies par l'utilisateur pour ses trajets
void menuPreference (Images Ima2, samples  Samp4, sound * Music,Graphe& grapho)
{
    ...
    VectorName.push_back("  Chemin prefe"); // pour question 4.5
    VectorName.push_back("  Modif prefe"); // pour question 4.5
    //VectorName.push_back("Modif param");
    ...
    while (exit_condition == 0)
    {
        ...
        if ( ExitResponseAccept == true )
        {
            ...
            {
            case 0:
                grapho.bufferParcour(Ima2,Samp4,Music,true);
                break;

            case 1:
                grapho.modifValPreference(Ima2,Samp4,Music);
                break;
